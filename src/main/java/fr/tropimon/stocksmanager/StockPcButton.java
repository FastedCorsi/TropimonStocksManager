...
final class StockPcButton extends ButtonWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        drawBackground(context, getX(), getY(), width, height, isHovered(), selected);
        var renderer = MinecraftClient.getInstance().textRenderer;
        context.drawCenteredTextWithShadow(renderer, getMessage(), getX() + width / 2,
                getY() + (height - 8) / 2, 0xFFFFFFFF);
    }

    /** Rend le véritable bouton du PC Cobblemon pour les actions uniquement graphiques. */
    static void drawBackground(DrawContext context, int x, int y, int width, int height,
                               boolean hovered, boolean selected) {
        int sourceY = hovered || selected ? SOURCE_HEIGHT : 0;
        int cap = Math.min(CAP_WIDTH, width / 2);
        int middle = Math.max(0, width - cap * 2);
        context.drawTexture(TEXTURE, x, y, cap, height,
                0, sourceY, CAP_WIDTH, SOURCE_HEIGHT, TEXTURE_WIDTH, TEXTURE_HEIGHT);
        if (middle > 0) {
            context.drawTexture(TEXTURE, x + cap, y, middle, height,
                    CAP_WIDTH, sourceY, TEXTURE_WIDTH - CAP_WIDTH * 2, SOURCE_HEIGHT,
                    TEXTURE_WIDTH, TEXTURE_HEIGHT);
        }
        context.drawTexture(TEXTURE, x + width - cap, y, cap, height,
