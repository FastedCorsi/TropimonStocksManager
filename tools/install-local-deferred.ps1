...
$jars = @(Get-ChildItem -LiteralPath $PSScriptRoot -Filter 'TropimonStocksManager-*-LOCAL.jar' -File)
...
& (Join-Path $PSScriptRoot 'InstallManagedLocalMod.ps1') -SourceJar $jars[0].FullName `
    -ExpectedModId 'tropimon_stocks_manager' -LauncherRoot $LauncherRoot -PollSeconds $PollSeconds
exit $LASTEXITCODE
